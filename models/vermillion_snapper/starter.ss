...
-1 #_minyr_sdreport:  min year for sdreport outputs (-1 for styr); #_1948
-2 #_maxyr_sdreport:  max year for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2027
...
2 # F_std_units:  0 (skip); 1 (exploitation(Bio)); 2 (exploitation(Num)); 3 (sum(Apical_F's)); 4 (mean F for range of ages (numbers weighted)); 5 (unweighted mean F for range of ages)
...
-1 #_seed:  random number seed for bootstrap data (-1 to use long(time) as seed): # 1758552776
